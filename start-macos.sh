#!/bin/bash
# Misantropic — one-command quick start for macOS (Apple Silicon).
#
#   ./start-macos.sh
#
# Installs everything Misantropic needs via Homebrew, sets up a local Python
...
# that's already done.
#
# Why native (not Docker): Cookbook serves models on whatever machine Misantropic
...
PORT="${MISANTROPIC_PORT:-7860}"   # 7860, not 7000 — macOS AirPlay Receiver holds 7000.
...
echo "▶ Misantropic quick start for macOS"
...
if (exec 3<>"/dev/tcp/127.0.0.1/$PORT") 2>/dev/null; then
  ...
  echo "    MISANTROPIC_PORT=7900 ./start-macos.sh"
  exit 1
fi
...
echo "▶ Preparing Misantropic…"
MISANTROPIC_SKIP_RUN_HINT=1 ./venv/bin/python setup.py
...
# MISANTROPIC_NO_OPEN=1 (e.g. over SSH / headless).
POLLER_PID=""
if [ -z "$MISANTROPIC_NO_OPEN" ] && command -v open >/dev/null 2>&1; then
  (
    for _ in $(seq 1 90); do
      if (exec 3<>"/dev/tcp/127.0.0.1/$PORT") 2>/dev/null; then
        printf '\n'
        printf '  ┌────────────────────────────────────────────┐\n'
        printf '  │  ✓ Misantropic is ready — opening your browser  │\n'
        printf '  │     %-40s │\n' "$URL"
...
echo
echo "▶ Starting Misantropic — it will open in your browser at $URL"
