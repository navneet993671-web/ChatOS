...
def _local_tooling_path_export(executable: str) -> str:
    ...
    When Misantropic runs from a virtualenv, that bin dir holds the tools the
...
def _append_serve_preflight_exit_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('if [ -n "$MISANTROPIC_PREFLIGHT_EXIT" ]; then')
    runner_lines.append('  echo ""; echo "=== Process exited with code $MISANTROPIC_PREFLIGHT_EXIT ==="')
    if keep_shell_open:
        runner_lines.append('  exec "${SHELL:-/bin/bash}"')
    else:
        runner_lines.append('  exit "$MISANTROPIC_PREFLIGHT_EXIT"')
...
def _append_serve_exit_code_lines(runner_lines: list[str], *, keep_shell_open: bool) -> None:
    ...
    runner_lines.append('MISANTROPIC_CMD_EXIT=$?')
    if keep_shell_open:
        runner_lines.append('echo ""; echo "=== Process exited with code $MISANTROPIC_CMD_EXIT ==="; exec "${SHELL:-/bin/bash}"')
    else:
        runner_lines.append('echo ""; echo "=== Process exited with code $MISANTROPIC_CMD_EXIT ==="')
...
WIN_SESSION_DIR = "$env:TEMP\\\\misantropic-sessions"
