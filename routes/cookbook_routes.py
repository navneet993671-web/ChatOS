...
_HF_TOKEN_STATUS_SNIPPET = (
    'if [ -n "$HF_TOKEN" ]; then '
    'echo "[misantropic] HF token: applied"; '
    'else '
    'echo "[misantropic] HF token: NOT SET — gated/private models will be denied. '
    'Add one in Misantropic Settings -> Cookbook -> HuggingFace Token."; '
    'fi'
)
...
def setup_cookbook_routes() -> APIRouter:
    ...
    async def generate_cookbook_ssh_key(request: Request):
        ...
        if not key_path.exists():
            ...
            proc = await asyncio.create_subprocess_exec(
                ssh_keygen, "-t", "ed25519", "-N", "", "-C", "misantropic-cookbook", "-f", str(key_path),
    ...
    def _user_shell_path_bootstrap() -> list[str]:
        return [
            'MISANTROPIC_USER_SHELL="${SHELL:-}"',
            'if [ -n "$MISANTROPIC_USER_SHELL" ] && [ -x "$MISANTROPIC_USER_SHELL" ]; then',
            '  MISANTROPIC_USER_PATH="$("$MISANTROPIC_USER_SHELL" -ic \'printf "__MISANTROPIC_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__MISANTROPIC_PATH__//p\' | tail -n 1 || true)"',
            '  if [ -n "$MISANTROPIC_USER_PATH" ]; then export PATH="$MISANTROPIC_USER_PATH:$PATH"; fi',
            'fi',
        ]
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        ...
        # When Misantropic runs from a venv (e.g. native macOS install), put its bin
        ...
        if remote and is_windows:
            ...
            ps_lines = []
            ps_lines.append('$sessionDir = "$env:TEMP\\misantropic-sessions"')
            ...
            launch_ps = (
                "$sd = \\\"$env:TEMP\\misantropic-sessions\\\"; "
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            runner_lines = ["#!/bin/bash"]
            runner_lines.extend(_user_shell_path_bootstrap())
            runner_lines.append('MISANTROPIC_PREFLIGHT_EXIT=""')
            # Put Misantropic's own venv bin on PATH (local runs only) so the serve
            ...
            if "llama_cpp" in req.cmd or "llama-server" in req.cmd:
                ...
                runner_lines.append('      echo "[misantropic] CUDA nvcc found — building llama-server with CUDA (GPU) support..."')
                runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release -DGGML_CUDA=ON \\')
                runner_lines.append('        && cmake --build build -j"$NPROC" --target llama-server \\')
                runner_lines.append('        && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
                runner_lines.append('    else')
                runner_lines.append('      echo "[misantropic] WARNING: nvcc not found — building llama-server for CPU only."')
                runner_lines.append('      echo "[misantropic]   GPU inference will not be available for this llama.cpp build."')
                runner_lines.append('      echo "[misantropic]   To get a GPU build, first install vLLM via Cookbook -> Dependencies"')
                runner_lines.append('      echo "[misantropic]   (its CUDA wheels include nvcc), then re-launch this serve task."')
                ...
                runner_lines.append('  MISANTROPIC_PREFLIGHT_EXIT=127')
                runner_lines.append('fi')
                ...
                runner_lines.append('  MISANTROPIC_PREFLIGHT_EXIT=1')
                runner_lines.append('fi')
    ...
    async def server_setup(request: Request, req: SetupRequest):
        ...
        if platform == "windows":
            ...
            setup_script = (
                'powershell -Command "'
                "New-Item -ItemType Directory -Force -Path $env:TEMP\\misantropic-sessions | Out-Null; "
