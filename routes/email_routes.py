...
MISANTROPIC_MAIL_ORIGIN = "odysseus-ui"
...
def _apply_misantropic_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-Odysseus-Origin"] = MISANTROPIC_MAIL_ORIGIN
    if kind:
...
def setup_email_routes():
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the Misantropic marker header, but include the subject
                # fallback too. The fallback uses a distinct Misantropic prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = _imap_uid_search(
                    conn,
                    f'(OR HEADER X-Odysseus-Kind "reminder" SUBJECT "Reminder (Misantropic):"{from_clause})',
                )
    ...
    @router.delete("/misantropic/reminders")
    async def delete_misantropic_reminder_emails(
        account_id: str | None = Query(None),
        permanent: bool = Query(False),
        owner: str = Depends(require_owner),
    ):
        """Delete email messages stamped as Misantropic reminders."""
        if account_id:
        ...
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (Misantropic):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (Misantropic):")})'))
                            # Legacy reminders created before the Misantropic
            ...
            logger.error(f"delete_misantropic_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", odysseus_kind=None, misantropic_ref=None,
    ):
        ...
        _apply_misantropic_headers(outer, odysseus_kind or "scheduled", misantropic_ref)
        if in_reply_to:
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="misantropic.local")
        ...
        if req.odysseus_kind:
            _apply_misantropic_headers(outer, req.odysseus_kind)
